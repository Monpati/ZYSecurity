...
export const setTextPlaceholder = (schema: FormSchema): PlaceholderModel => {
  const personMap = [ComponentNameEnum.PERSON_INPUT]
  const cardMap = [ComponentNameEnum.CARD_INPUT]
  const corpNameMap = [ComponentNameEnum.CORP_NAME_INPUT]
  const corpNumMap = [ComponentNameEnum.CORP_NUM_INPUT]
  const lgpersonNameMap = [ComponentNameEnum.LGPERSON_NAME_INPUT]
  const lgpersonNumMap = [ComponentNameEnum.LGPERSON_NUM_INPUT]
  const textMap = [
  ...
  if (personMap.includes(schema?.component as ComponentNameEnum)) {
    return {
      placeholder: t('certification.nameIndex')
    }
  }
  if (cardMap.includes(schema?.component as ComponentNameEnum)) {
    return {
      placeholder: t('certification.idIndex')
    }
  }
  if (corpNameMap.includes(schema?.component as ComponentNameEnum)) {
    return {
      placeholder: t('certification.corpNameIndex')
    }
  }
  if (corpNumMap.includes(schema?.component as ComponentNameEnum)) {
    return {
      placeholder: t('certification.corpNumIndex')
    }
  }
  if (lgpersonNameMap.includes(schema?.component as ComponentNameEnum)) {
    return {
      placeholder: t('certification.lgPersonNameIndex')
    }
  }
  if (lgpersonNumMap.includes(schema?.component as ComponentNameEnum)) {
    return {
      placeholder: t('certification.lgPersonCardIdIndex')
    }
  }
