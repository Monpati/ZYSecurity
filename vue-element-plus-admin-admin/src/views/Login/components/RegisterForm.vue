...
import { UserReg } from '@/api/login/types'
...
const getCode = () => {
  ...
  axios
    .get('http://localhost:8080/v1/getcode')
    .then((response) => {
      console.log('Get code request successful:', response)
    })
    .catch((error) => {
      console.error('Error while sending get code request:', error)
    })
    .finally(() => {
...
const schema = reactive<FormSchema[]>([
  ...
  },
  {
    ...
    componentProps: {
      placeholder: t('login.telephoneNumberPlaceholder')
    }
  },
  {
    field: 'code',
...
const loginRegister = async () => {
  ...
  formRef?.validate(async (valid) => {
    if (valid) {
      loading.value = true
      const formData = await getFormData<UserReg>()
      try {
        loading.value = true
        regApi(formData)
        toLogin()
