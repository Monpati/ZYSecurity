...
func (BWInstance) TableName() string {
	return "ScdnBWInstance"
}

func CreateBWInstance(db *gorm.DB, info *form.BWInfo) error {
	items := db.Table("ScdnBWInstance")
	sf, _ := utils.NewSnowflake(utils.GenerateRand())
	return items.Create(&BWInstance{
		Id:         sf.Generate(),
		BwId:       info.BwId,
...
func (p *BWInstance) GetBWInstanceLists(db *gorm.DB, info *form.BWInfoFilterForm) (*[]BWInstance, int, error) {
...
func (p *BWInstance) UpdateBWInstance(db *gorm.DB, id int64, info *form.BWInfo) error {
